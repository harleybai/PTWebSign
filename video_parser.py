...
def parser(html, link):
    ...
    if re.search(r"www.bilibili.com", link):
        ...
        res["pic"] = (
            soup.find("div", {"class": "review-edit-header"}).find("img").get("src")
        )
    ...
    # 2. 腾讯视频
        ...
        res["pic"] = (
            soup.find("div", {"class": "container_inner"})
            .find("img", {"class": "figure_pic"})
            .get("src")
        )
        if soup.find("a", {"class": "score_db"}):
            res["douban"] = soup.find("a", {"class": "score_db"}).get("href")
        for s in soup.find("div", {"class": "video_type video_type_even cf"}).findAll(
            "span", {"class": "type_txt"}
        ):
    ...
    # 3. 豆瓣
        ...
        res["date"] = (
            soup.find("span", {"class": "pl"}, True, "上映日期:")
            .findNextSibling()
            .getText()
        )
        res["douban"] = link
